#ifndef SHADER_CUH
#define SHADER_CUH

#include "core/vec3.cuh"
#include "core/ray.cuh"
#include "core/material.cuh"
#include "geometry/quad.cuh"
#include "rendering/light.cuh"

/**
 * @file shader.cuh
 * @brief Shading helpers: hit payload, light sampling, shadows, and Lambert shading.
 */

// ========================
// Hit payload
// ========================
struct Hit {
    float t; ///< Ray parameter at hit
    Vec3 P; ///< World-space hit point
    Vec3 N; ///< Geometric normal (unit)
    Material mat; ///< Material at hit
    bool hit; ///< Did we hit anything?
};

// ========================
// Small color helpers
// ========================
__host__ __device__ inline Vec3 toFloat3(const uchar3 c) {
    return Vec3(c.x, c.y, c.z) / 255.0f;
}

__host__ __device__ inline uchar3 toUChar3(const Vec3 &c) {
    auto clamp01 = [](float v) { return fminf(fmaxf(v, 0.0f), 1.0f); };
    return make_uchar3(
        static_cast<unsigned char>(255.0f * clamp01(c.x)),
        static_cast<unsigned char>(255.0f * clamp01(c.y)),
        static_cast<unsigned char>(255.0f * clamp01(c.z))
    );
}

// Gamma encode (approx sRGB) for nicer output previews
__host__ __device__ inline Vec3 gammaEncode(const Vec3 &linear) {
    return Vec3(sqrtf(linear.x), sqrtf(linear.y), sqrtf(linear.z));
}

// ========================
// Light sampling (dir + attenuation)
// ========================
__host__ __device__ inline void sampleLight(
    const Light &light, const Vec3 &P,
    Vec3 &L, float &attenuation, float &distToLight) {
    if (light.type == POINT) {
        const Vec3 toL = light.position - P;
        distToLight = toL.length();
        L = (distToLight > 0.0f) ? (toL / distToLight) : Vec3(0.0f);
        // Simple inverse-square falloff with small floor to avoid INF
        attenuation = 1.0f / fmaxf(distToLight * distToLight, 1e-3f);
    } else if (light.type == DIRECTIONAL) {
        L = (-light.direction).normalize();
        attenuation = 1.0f;
        distToLight = 1e30f; // effectively infinite
    } else {
        // SPOT → for now behave like point; cone factor can be added later
        const Vec3 toL = light.position - P;
        distToLight = toL.length();
        L = (distToLight > 0.0f) ? (toL / distToLight) : Vec3(0.0f);
        attenuation = 1.0f / fmaxf(distToLight * distToLight, 1e-3f);
        // TODO: add cone factor using dot(light.direction, normalize(P - light.position))
    }
}

// ========================
// Shadow test against quads
// Cast Rshadow = (P + eps*N) → light; return true if blocked.
// ========================
__host__ __device__ inline bool isOccludedByQuads(
    const Vec3 &P, const Vec3 &N, const Vec3 &L, const float maxDist,
    const Quad *quads, const int numQuads) {
    constexpr float kEps = 1e-3f; // avoid self-intersection
    Ray shadowRay(P + N * kEps, L);

    for (int i = 0; i < numQuads; ++i) {
        float t;
        if (quads[i].intersect(shadowRay, t) && t > 0.0f && t < maxDist - 1e-4f) {
            return true; // blocked
        }
    }
    return false;
}

// ========================
// Lambert shading (with hard shadows + ambient + gamma)
// ========================
__host__ __device__ inline Vec3 shadeLambert(
    const Hit &h,
    const Light &light,
    const Quad *quads, const int numQuads,
    const Vec3 &ambient = Vec3(0.03f, 0.03f, 0.03f)) {
    if (!h.hit) return Vec3(0.0f); // caller should handle background

    // Light sample
    Vec3 L;
    float att = 1.0f;
    float dist = 1e30f;
    sampleLight(light, h.P, L, att, dist);

    // Early ambient if in shadow
    if (isOccludedByQuads(h.P, h.N, L, dist, quads, numQuads)) {
        const Vec3 base = toFloat3(h.mat.color);
        const Vec3 color = base * ambient;
        return gammaEncode(color);
    }

    // BRDF terms
    const float nDotL = fmaxf(0.0f, h.N.dot(L));
    const Vec3 base = toFloat3(h.mat.color);
    const Vec3 Li = toFloat3(light.color) * light.intensity * att;

    // Lambert + ambient
    const Vec3 color = base * nDotL * Li + base * ambient;

    return gammaEncode(color);
}

#endif // SHADER_CUH
